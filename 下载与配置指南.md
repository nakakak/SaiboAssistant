...
| 检查项 | 必须？ | 说明 | 如何确认 / 调整 |
|--------|--------|------|-----------------|
| 商家云已运行 | ✅ | 公网或局域网 `anime-ai-chat-server` | 浏览器能打开管理台，如 `https://xiaozhi.cyberai.top` |
...
| 电脑能访问商家云 | ✅ | 助手与云端互通 | 本机 `curl -sI https://xiaozhi.cyberai.top` 有响应 |
...
|--------|--------|------|------------|-----------------|
| **`server_url`** | ✅ | 连商家云 Connector 的 WebSocket | 内置默认 `wss://xiaozhi.cyberai.top/bridge/connector`；局域网测试为 `ws://192.168.50.52:8084/bridge/connector` | 向导中为灰色只读；换云：**托盘 → 连接设置 → 商家云 WebSocket → 保存并重连**。须含路径 **`/bridge/connector`**，不要手写 `?token=` |
...
1. **确认商家云**（公网 `https://xiaozhi.cyberai.top` 或局域网 `192.168.50.52:8084`），`bridge_enabled: true`。
...
5. **向导第 2 步**：
   - 商家云地址：默认已填 `wss://xiaozhi.cyberai.top/bridge/connector`（一般不用改）。
...
```yaml
server_url: "wss://xiaozhi.cyberai.top/bridge/connector"
...
| curl（可选） | `curl -s "https://xiaozhi.cyberai.top/bridge/connector?device_mac=你的MAC"` → `websocket upgrade failed` 表示桥接与设备校验通过 |
...
|------|----------|------------|----------|------|
| **v0.2.17+**（推荐） | [GitHub Release v0.2.17](https://github.com/nakakak/SaiboAssistant/releases/tag/v0.2.17) 或源码构建 | ✅ | ✅ | 当前 **Latest**；内置 `wss://xiaozhi.cyberai.top/bridge/connector` |
| **v0.2.16** | [GitHub Release v0.2.16](https://github.com/nakakak/SaiboAssistant/releases/tag/v0.2.16) | ✅ | ✅ | 内置局域网 `ws://192.168.50.52:8084/bridge/connector` |
...
**公网商家云（本仓库默认内置）：** `wss://xiaozhi.cyberai.top/bridge/connector`  
**局域网测试：** `ws://192.168.50.52:8084/bridge/connector`
...
> 3. 公网商家云默认 **`wss://xiaozhi.cyberai.top/bridge/connector`**；局域网测试可在连接设置中改为 `ws://192.168.50.52:8084/bridge/connector`。
...
**v0.2.17+** 已内置公网地址 `wss://xiaozhi.cyberai.top/bridge/connector`，向导中一般为灰色只读。局域网测试时在 **连接设置** 中修改。
...
|------|----------------|--------------------------------------|
| **公网云（本仓库默认）** | https://xiaozhi.cyberai.top | **`wss://xiaozhi.cyberai.top/bridge/connector`** |
| 局域网测试 | http://192.168.50.52:8084 | `ws://192.168.50.52:8084/bridge/connector` |
| 局域网测试（其它 IP） | http://192.168.50.176:8084 | `ws://192.168.50.176:8084/bridge/connector` |
...
| [v0.2.17（当前 Latest）](https://github.com/nakakak/SaiboAssistant/releases/tag/v0.2.17) | 预编译包，内置公网云 |
| [v0.2.16](https://github.com/nakakak/SaiboAssistant/releases/tag/v0.2.16) | 预编译包，内置 192.168.50.52 |
| [v0.2.15](https://github.com/nakakak/SaiboAssistant/releases/tag/v0.2.15) | 预编译包，内置 192.168.50.176 |
