...
| 检查项 | 必须？ | 说明 | 如何确认 / 调整 |
|--------|--------|------|-----------------|
| 商家云已运行 | ✅ | 本机或局域网 `anime-ai-chat-server` | 浏览器能打开管理台，如 `http://192.168.50.52:8084` |
...
| 电脑与云、设备同网 | ✅（局域网） | 助手、Gateway、商家云互通 | 本机 `curl http://192.168.50.52:8084/` 有响应 |
...
|--------|--------|------|------------|-----------------|
| **`server_url`** | ✅ | 连商家云 Connector 的 WebSocket | 内置默认 `ws://192.168.50.52:8084/bridge/connector`；公网为 `wss://xiaozhi.cyberai.top/bridge/connector` | 向导中为灰色只读；局域网/换云：**托盘 → 连接设置 → 商家云 WebSocket → 保存并重连**。须含路径 **`/bridge/connector`**，不要手写 `?token=` |
...
1. **启动商家云**（本例 `192.168.50.52:8084`），确认 `bridge_enabled: true`。
...
5. **向导第 2 步**：
   - 商家云地址：默认已填 `ws://192.168.50.52:8084/bridge/connector`（一般不用改）。
...
```yaml
server_url: "ws://192.168.50.52:8084/bridge/connector"
...
| curl（可选） | `curl -s "http://192.168.50.52:8084/bridge/connector?device_mac=你的MAC"` → `websocket upgrade failed` 表示桥接与设备校验通过 |
...
|------|----------|------------|----------|------|
| **v0.2.16+**（推荐） | [GitHub Release v0.2.16](https://github.com/nakakak/SaiboAssistant/releases/tag/v0.2.16) 或源码构建 | ✅ | ✅ | 当前 **Latest**；内置 `ws://192.168.50.52:8084/bridge/connector` |
...
**局域网商家云（本仓库默认内置）：** `ws://192.168.50.52:8084/bridge/connector`  
...
> 3. 局域网商家云默认 **`ws://192.168.50.52:8084/bridge/connector`**；公网可在连接设置中改为 `wss://xiaozhi.cyberai.top/bridge/connector`。
...
**v0.2.16+** 已内置局域网地址 `ws://192.168.50.52:8084/bridge/connector`，向导中一般为灰色只读。换云时在 **连接设置** 中修改。
...
| 局域网测试（本仓库默认） | http://192.168.50.52:8084 | `ws://192.168.50.52:8084/bridge/connector` |
...
| [v0.2.16（当前 Latest）](https://github.com/nakakak/SaiboAssistant/releases/tag/v0.2.16) | 预编译包，内置 192.168.50.52 |
