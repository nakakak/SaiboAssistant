...
> **v0.2.14 配对说明（重要）**  
> 当前版本 **仅支持使用设备 MAC 地址** 连接商家云。  
> 向导或连接设置里若仍看到「连接码 / 验证码」选项，**请勿使用**；请改选 **「设备 MAC」** 并填写完整 MAC（如 `98:88:e0:14:79:a0`）。  
> **6 位连接码（验证码）配对** 计划在后续版本开放。
...
| 环境 | 管理台地址 | **SaiboAssistant 填写的 server_url** |
|------|------------|--------------------------------------|
| **公网云（推荐线上）** | https://xiaozhi.cyberai.top/admin | **`wss://xiaozhi.cyberai.top/bridge/connector`** |
| 局域网测试（如 192.168.50.52） | http://192.168.50.52:8084/admin | `ws://192.168.50.52:8084/bridge/connector` |
| 本机开发 | http://127.0.0.1:8084/admin | `ws://127.0.0.1:8084/bridge/connector` |
...
1. 登录商家云管理台（公网云示例：**https://xiaozhi.cyberai.top/admin**）。
...
管理台或设备上可能出现的 **6 位电脑连接码 / 验证码**，**当前版本助手不支持**，请勿在向导里选「连接码」配对。  
该能力计划在 **后续版本** 提供；届时文档会更新，连接码将可自动解析为 MAC。

<!-- 后续版本可参考的 API（当前助手未启用）：
...
| 用了连接码连不上 | **v0.2.14 仅支持 MAC**；改选「设备 MAC」并填管理台中的 MAC |
